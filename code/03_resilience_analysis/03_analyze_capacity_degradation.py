...
TOL_EPS = 1e-9
ALPHA_FIXED = 10.0
DEGRADATION_RATES = [0.0, 0.2, 0.4, 0.6, 0.8]
...
def process_single_query(args):
    (i, row_data, feasible_sups, disrupted_params, tol_caps, disruption_mode) = args
    ...
    num_alternatives = len(active)

    if (not active) or (demand <= 0):
        return {
            'eff_cost': float(ALPHA_FIXED),
            'eff_time': float(ALPHA_FIXED),
            'num_alternatives': num_alternatives,
            'fulfillment_rate': 0.0,
            'feasible_fulfillment_rate': 0.0,
            'unmet_volume': demand,
    ...
    def solve_mip(obj_type: str):
        ...
        lambda_val = float(ALPHA_FIXED) * float(max(coeffs.values()))
        ...
        return float(pred_val), float(u_cap), float(u_qual), x_sol

    p_c, uc_c, uq_c, x_sol_c = solve_mip("cost")
    p_t, uc_t, uq_t, x_sol_t = solve_mip("time")

    u_gt_c = (gt_c / demand) if demand > 0 else 0.0
    u_gt_t = (gt_t / demand) if demand > 0 else 0.0

    eff_c = (p_c + (uc_c + uq_c) * float(ALPHA_FIXED) * u_gt_c) / gt_c if gt_c > 0 else float(ALPHA_FIXED)
    eff_t = (p_t + (uc_t + uq_t) * float(ALPHA_FIXED) * u_gt_t) / gt_t if gt_t > 0 else float(ALPHA_FIXED)

    fulfilled_qty = sum(x_sol_c.values())
    feasible_qty = sum(qty for s, qty in x_sol_c.items() if tol_exact_match(s))
    fulfillment_rate = (fulfilled_qty / demand) if demand > 0 else 0.0
    feasible_fulfillment_rate = (feasible_qty / demand) if demand > 0 else 0.0
    unmet_total = uc_c + uq_c

    return {
        'eff_cost': float(eff_c),
        'eff_time': float(eff_t),
        'num_alternatives': num_alternatives,
        'fulfillment_rate': float(fulfillment_rate),
        'feasible_fulfillment_rate': float(feasible_fulfillment_rate),
        'unmet_volume': float(unmet_total),
        'disruption_mode': disruption_mode
    }
...
def run_resilience_analysis():
    ...
    for mode in models:
        print(f"\n{'=' * 60}")
        print(f'[Model: {mode.upper()}] Processing...')
        print(f"{'=' * 60}")
        ...
        disruption_mode = 'heterogeneous_degradation'
        print(f'\n  > Disruption Mode: {disruption_mode.upper()}')
        for rate in DEGRADATION_RATES:
            print(f'    [Max Degradation {int(rate * 100):2d}%]: ', end='', flush=True)
            seeds_c, seeds_t = ([], [])
            seeds_alt, seeds_fulf, seeds_feas, seeds_unmet = ([], [], [], [])
            for seed_idx in range(NUM_SEEDS):
                ...
                degradation_rates = {s: np.random.uniform(0, rate) for s in all_sups}
                for k in disrupted_params:
                    supplier_id = k[0]
                    degradation = degradation_rates[supplier_id]
                    disrupted_params[k]['capacity'] *= 1 - degradation
                tasks = []
                for i in range(len(test_df)):
                    ...
                    feas = feasible_map[i]
                    tasks.append((i, row_data, feas, disrupted_params, tol_caps, disruption_mode))
                with Pool(processes=num_cores) as pool:
                    results = pool.map(process_single_query, tasks)
                valid_c = [r['eff_cost'] for r in results if not np.isnan(r['eff_cost'])]
                valid_t = [r['eff_time'] for r in results if not np.isnan(r['eff_time'])]
                valid_alt = [r['num_alternatives'] for r in results]
                valid_fulf = [r['fulfillment_rate'] for r in results]
                valid_feas = [r['feasible_fulfillment_rate'] for r in results]
                valid_unmet = [r['unmet_volume'] for r in results]
                if valid_c:
                    seeds_c.append(float(np.mean(valid_c)))
                if valid_t:
                    seeds_t.append(float(np.mean(valid_t)))
                seeds_alt.append(float(np.mean(valid_alt)))
                seeds_fulf.append(float(np.mean(valid_fulf)))
                seeds_feas.append(float(np.mean(valid_feas)))
                seeds_unmet.append(float(np.mean(valid_unmet)))
                print('.', end='', flush=True)
            final_results.append({'Model': mode, 'Disruption_Mode': disruption_mode, 'Max_Degradation_Rate': rate, 'Eff_Cost_Ratio_Mean': float(np.mean(seeds_c)) if seeds_c else np.nan, 'Eff_Cost_Ratio_Std': float(np.std(seeds_c)) if seeds_c else np.nan, 'Eff_Time_Ratio_Mean': float(np.mean(seeds_t)) if seeds_t else np.nan, 'Eff_Time_Ratio_Std': float(np.std(seeds_t)) if seeds_t else np.nan, 'Num_Alternatives_Mean': float(np.mean(seeds_alt)), 'Num_Alternatives_Std': float(np.std(seeds_alt)), 'Fulfillment_Rate_Mean': float(np.mean(seeds_fulf)), 'Feasible_Fulfillment_Rate_Mean': float(np.mean(seeds_feas)), 'Feasible_Fulfillment_Rate_Std': float(np.std(seeds_feas)), 'Unmet_Volume_Mean': float(np.mean(seeds_unmet)), 'Unmet_Volume_Std': float(np.std(seeds_unmet))})
            print(f' Done (Cost: {np.mean(seeds_c):.3f}, Fulfillment: {np.mean(seeds_fulf):.1%})')
    res_df = pd.DataFrame(final_results)
    out_path = BASE_DIR / 'results' / '03_resilience_analysis' / 'heterogeneous_capacity_degradation.csv'
